...
export interface ColumnDefinitionConfig {
  ...
  /** Product MBOM: row can disconnect SKU only when not MBOM and SpecSheet Extra is not Yes */
...
export class GridService {
  ...
  flattenHierarchicalData(data: any[], config: HierarchicalDataConfig): any[] {
    ...
    const processNode = (node: any) => {
      ...
      if (this.shouldSkipAlternateBomNode(node, isAlternateBom)) {
  ...
  private shouldSkipAlternateBomNode(node: any, isAlternateBom: boolean): boolean {
    if (!isAlternateBom) return false;
    ...
    const filterRows = (rows: any[]): any[] => {
      return rows
        .map((row) => {
          ...
          if (this.shouldSkipAlternateBomNode(row, isAlternateBom) || !this.shouldIncludeNode(row)) {
            return null;
          }
  ...
  createSkuColumns(config: ColumnDefinitionConfig): ColDef[] {
    ...
    const useReleasedForEditable = false;
